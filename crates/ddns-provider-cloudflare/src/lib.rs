...
impl CloudflareProvider {
    ...
    async fn get_zone_id(&self, domain: &str) -> Result<String> {
        ...
        let zone_id = zone["id"].as_str().ok_or_else(|| {
            Error::provider(
                "cloudflare",
                "Invalid response format: zone.id is not a string",
            )
        })?;
    ...
    async fn get_record_id(
        ...
        let record_id = record["id"].as_str().ok_or_else(|| {
            Error::provider(
                "cloudflare",
                "Invalid response format: record.id is not a string",
            )
        })?;
...
impl DnsProvider for CloudflareProvider {
    ...
    async fn update_record(&self, record_name: &str, new_ip: IpAddr) -> Result<UpdateResult> {
        ...
        let current_ip_str = record_json["result"]["content"].as_str().ok_or_else(|| {
            Error::provider(
                "cloudflare",
                "Invalid response format: content is not a string",
            )
        })?;
...
impl ProviderConfigurable for CloudflareConfigurable {
    ...
    ///
    /// Reads:
    /// - `DDNS_CLOUDFLARE_API_TOKEN` (required)
    /// - `DDNS_CLOUDFLARE_ZONE_ID` (optional)
    /// - `DDNS_CLOUDFLARE_ACCOUNT_ID` (optional)
    fn load_from_env(&self) -> Result<Value> {
        let api_token = std::env::var("DDNS_CLOUDFLARE_API_TOKEN")
            .map_err(|_| Error::config("DDNS_CLOUDFLARE_API_TOKEN is required"))?;

        if api_token.is_empty() {
            return Err(Error::config("DDNS_CLOUDFLARE_API_TOKEN cannot be empty"));
        }

        let zone_id = std::env::var("DDNS_CLOUDFLARE_ZONE_ID").ok();
        let account_id = std::env::var("DDNS_CLOUDFLARE_ACCOUNT_ID").ok();
...
mod tests {
    ...
    fn test_http_timeout_configured() {
        ...
        let provider = CloudflareProvider::new("test_token", None, None, false);
